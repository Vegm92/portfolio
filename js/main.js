...
async function renderProjects() {
    ...
    track.innerHTML = '<div style="color: var(--ink-dim);">Loading projects...</div>';

    try {
        const response = await fetch('to_publish/projects.json');
        const data = await response.json();
        
        // --- Dynamic Metadata ---
        const numProjects = data.projects.length;
        if ($('#aboutShipped')) $('#aboutShipped').textContent = `${numProjects} projects`;
        ...
        let techCounts = {};
        allTech.forEach(t => { techCounts[t] = (techCounts[t] || 0) + 1; });
        let sortedTech = Object.keys(techCounts).sort((a,b) => techCounts[b] - techCounts[a]);

        const stackSet = new Set([
            'typescript', 'python', 'rust', 'javascript', 'ts', 'js',
            'react', 'next.js', 'sveltekit', 'drizzle orm', 'sqlite', 'tailwind css'
        ]);
        const aiSet = new Set(['gemini ai', 'claude sdk', 'claude api', 'muapi.ai', 'mcp', 'elevenlabs', 'claude']);
        
        let stack = [], aiTools = [], tools = [];
        sortedTech.forEach(t => {
            const lower = t.toLowerCase();
            if (aiSet.has(lower) || lower.includes('ai') || lower.includes('claude')) aiTools.push(t);
        ...
        const carousel = new Carousel("#carouselTrack", "#carPrev", "#carNext", "#carouselIdx");
        const carItems = [];
        ...
        projects.forEach((project, index) => {
            ...
            if (project.screenshot) {
                ...
                img.src = `to_publish/${project.screenshot}`;
                img.onload = () => img.classList.add('loaded');
                // Replace placeholder completely if we have an image
                shotDiv.innerHTML = ''; 
                shotDiv.appendChild(img);
            } else {
                clone.querySelector('.shot-filename').textContent = `${project.name.toLowerCase()}.png`;
            }
        ...
        const track = $("#carouselTrack");
        if (track) {
            track.innerHTML = '<div style="color: #ffb547;">Error loading projects. Make sure to run the local server.</div>';
        }
    }
}
